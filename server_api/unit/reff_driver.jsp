...
<%
    /**
     * Deploy: {webapp}/trucking/mobile/api/unit/reff_driver.jsp
     * GET method=list-driver&limit=10&offset=0&search=
     */
    ...
    try {
        ...
        if (!"list-driver".equalsIgnoreCase(method)) {
        ...
        if (!search.isEmpty()) {
            whereClause = "UPPER(DRVID) LIKE UPPER('%" + search + "%') "
                    + " OR UPPER(DRVNAME) LIKE UPPER('%" + search + "%')";
        }

        String sqlCount = "SELECT COUNT(*) AS CNT FROM TBLDRIVER WHERE " + whereClause;
        int total = queryCount(db, sqlCount);

        String cols = "DRVID AS value, DRVNAME AS title";
        String dataSql = "SELECT " + cols + " FROM TBLDRIVER WHERE " + whereClause + " ORDER BY DRVNAME ASC";
