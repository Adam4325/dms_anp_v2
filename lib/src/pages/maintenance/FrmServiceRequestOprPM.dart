              ...
              itemBuilder: (context, index) {
                return GestureDetector(
                    onTap: () async {
                      ...
                      txtOpnameVHCID.text =
                          listVehicleId[index]['value'].toString();
                      var locidVEH = listVehicleId[index]['title'].toString();
                      if (locidVEH != null && locidVEH != "") {
                        print('locidVEH ${locidVEH}');
                        var splitVEH = locidVEH.split("<=>");
                        if (splitVEH != null && splitVEH.length > 0) {
                          print("splitVEH ${splitVEH}");
                          var a = splitVEH[1].toString();
                          txtCabangId.text = a.trim();
                        }
                      }
                    },
                    child: Container(
...
class FrmServiceRequestOprPM extends StatefulWidget {
  @override
...
class _FrmServiceRequestOprPMState extends State<FrmServiceRequestOprPM>
  ...
  String btnNameCreatePR = "Create PR";
  ...
  void _applySparepartSelection(dynamic item, {String? itemIdOverride}) {
    ...
    setState(() {
      if (service_typeid == "PM1" ||
        ...
        if (qty != null && qty.toString().isNotEmpty) {
          txtOpnameQty.text = qty.toString();
        }
      }
      final itemId =
          (itemIdOverride ?? item['item_id']?.toString() ?? '').trim();
      final partName = (item['part_name']?.toString() ?? '').trim();
      ...
      nama_katalog = selKatalog.isEmpty ? '' : '(${_katalogTitleOf(selKatalog)})';
    });
  }
  ...
  Future<String> getMenuKatalog() async {
    String status = "";
    var urlData =
        "${BASE_URL}api/maintenance/sr/menu_opname.jsp?method=menu-sr&service_typeid=$service_typeid&merk=$pm_merk&vhttype=$pm_vhttype";
  ...
  Future getListSR() async {
    Uri myUri = Uri.parse(
        "${BASE_URL}api/maintenance/sr/refferencce_mcn.jsp?method=list_typeservice-pm");
    print(myUri.toString());
    var response =
        await http.get(myUri, headers: {"Accept": "application/json"});
  ...
  void getVehicleListOld(String cabangId) async {
  ...
  void getVehicleListNonLocid(String cabangId) async {
    if (is_edit_req == true) return;
    try {
      var urlData =
          "${BASE_URL}api/maintenance/sr/list_vehicle.jsp?method=lookup-vehicle-v2&locid=";
      var encoded = Uri.encodeFull(urlData);
      print(urlData);
      Uri myUri = Uri.parse(encoded);
      var response =
    ...
    setState(() {
      if (response.statusCode == 200) {
        ...
        if (result != null && result.length > 0) {
          _applySparepartSelection(result[0], itemIdOverride: itemid);
  ...
  Future getItemByText(String itemIDs) async {
    setState(() {
      ...
      if (METHOD_DETAIL == "PURCHASE-ORDER") {
        urlBase =
            "${BASE_URL}api/inventory/list_item_barcode_mobile.jsp?method=list-purchase-order-v1&warehouseid=${globals.from_ware_house}&search=$itemID&is_barcode=1";
      } else {
        urlBase =
    ...
    try {
      ...
      if (fnVHCID == null || fnVHCID == "") {
        ...
        alert(globalScaffoldKey.currentContext!, 0, "KM tidak boleh kosong",
            "error");
      } else if (double.parse(fnSTARTKM) <= 0) {
        ...
        alert(globalScaffoldKey.currentContext!, 0, "Price tidak boleh kosong",
            "error");
      } else if (double.parse(fnTYREPRICE) <= 0) {
  ...
  void createOpnameDev() async {
    id_header = 2355;
    METHOD_DETAIL = 'OPNAME';
    alert(globalScaffoldKey.currentContext!, 3, "Dev Success", "info");
  }
  ...
  void DeleteOpnameSrDetail(int id_header, int id_detail, String vhcid) async {
    try {
      if (id_header <= 0) {
        ...
        print('Create New OPNAME');
        var encoded = Uri.encodeFull(
            "${BASE_URL}api/maintenance/sr/delete_opname_sr_detail.jsp");
        print(encoded);
        Uri urlEncode = Uri.parse(encoded);
        var data = {
          'method': "delete-opname-detail-foreman",
          'id_header': id_header.toString(),
          'id': id_detail.toString(),
          'vhcid': vhcid,
          'userid': userid.toUpperCase(),
          'company': 'AN'
        };
  ...
  Future getListDataOpnameDetail(
    ...
    try {
      ...
      // if (METHOD_DETAIL == "PURCHASE-ORDER") {
      //   urlBase =
      //       "${BASE_URL}api/maintenance/sr/list_opname_sr_detail.jsp?method=list-purchase-order&id_header=${id_header}&vhcid=${vehicle_id}";
      // } else {
      //   urlBase =
  ...
  Future<List<Map<String, dynamic>>> _fetchItemList(String search,
    ...
    final method = METHOD_DETAIL == "PURCHASE-ORDER"
        ...
        : "list-items-v1";
    final url = Uri.parse(
        "${BASE_URL}api/inventory/list_item_sr_katalog_new.jsp?method=$method"
  ...
  Future<void> _openItemSearchDialog({String search = ''}) async {
    // Info dialog / EasyLoading overlay must finish popping first.
...
//PROSESS
  ...
  Widget _buildDListDetailOpname(dynamic item, int index) {
    return _pmListCard(
      compact: true,
      title:
          "Katalog : ${_katalogTitleOf((item['katalog'] ?? item['itpid'] ?? '').toString())}",
      rows: [
        _kv("ItemID", _s(item['itemid']), dense: true),
        _kv("TypID", _s(item['idtype']), dense: true),
        _kv("Genuino", _s(item['genuineno']), dense: true),
        _kv("Merk", _s(item['merk']), dense: true),
        _kv("Partname", _s(item['partname']), dense: true),
        _kv("Item Size", _s(item['item_size']), dense: true),
        _kv("QTY", _s(item['qty']), dense: true),
      ],
      actions: Row(
        children: [
  ...
  Widget _buildDListDetailOpnameSr(dynamic item, int index) {
    return _pmListCard(
      ...
      rows: [
        _kv("SR DateTime", _fmtDt(item['requestdate']), dense: true),
        _kv("VHCID", _s(item['vhcid']), dense: true),
        _kv("LOCID", _s(item['srlocid']), dense: true),
        _kv("DRV. NAME", _s(item['drvname']), dense: true),
        _kv("NOTES", _s(item['srnotes']), dense: true),
      ],
      actions: Row(
        children: [
          _pmBtn(
            ...
            onPressed: () async {
              ...
              showDialog(
                ...
                builder: (context) => pmAlertDialog(
                  title: 'Information',
                  content: const Text("Save Opname ?"),
                  actions: [
                    ElevatedButton.icon(
                      icon: const Icon(Icons.save, color: Colors.white, size: 16),
                      label: pmBtnLabel("Save Opname"),
  ...
  Widget listDataOpnameDetail(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: dataListOpnameDetail.isEmpty
          ? Center(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
          ...
          : ListView.builder(
              ...
              itemCount: dataListOpnameDetail.length,
              itemBuilder: (context, index) {
  ...
  Widget listDataSrOpname(BuildContext context) {
    ...
    return StatefulBuilder(
      builder: (context, setDialogState) {
        final items = List<Map<String, dynamic>>.from(dataListSrOpname);
        Future<void> _reload(String raw) async {
          final q = raw.trim();
          if (q.isEmpty || METHOD_DETAIL != '') return;
          await getListDataSr(true, q.split('/')[0]);
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  ElevatedButton(
                    onPressed: () => _reload(txtSearchVehicleSr.text),
                    style: pmBtnStyle(primaryOrange),
              ...
              Expanded(
                child: items.isEmpty
                    ...
                    : ListView.builder(
                        key: ValueKey('sr-opname-${items.length}'),
                        padding: const EdgeInsets.only(bottom: 4),
  ...
  Widget setupAlertDialoagContainerOpnamDetail(BuildContext context) {
    final size = MediaQuery.of(context).size;
    final items = List<Map<String, dynamic>>.from(dataListItemSearch);
    return SizedBox(
      width: size.width,
      height: size.height * 0.62,
      child: Column(
        children: [
          ...
          Expanded(
            child: items.isEmpty
                ...
                : ListView.builder(
                    key: ValueKey('opname-detail-${items.length}'),
                    padding: const EdgeInsets.only(bottom: 4),
                    itemCount: items.length,
                    itemBuilder: (context, index) {
                      if (index < 0 || index >= items.length) {
                      ...
                      return _buildDListTempOpnameDetails(items[index], index);
                    },
                  ),
          ),
        ],
  ...
  Future<void> _showOpnameDetailDialog(BuildContext context) async {
    await showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: Row(
            children: [
              Icon(Icons.inventory_2_outlined, color: darkOrange, size: 22),
              const SizedBox(width: 8),
              const Expanded(
                child: Text(
                  'List Item',
          ...
          content: setupAlertDialoagContainerOpnamDetail(context),
          actions: [
            ElevatedButton.icon(
              icon: const Icon(Icons.close, color: Colors.white, size: 16),
              label: pmBtnLabel("Close"),
                ...
                child: Row(children: <Widget>[
                  ...
                  Expanded(
                      child: ElevatedButton.icon(
                        ...
                        label: pmBtnLabel("${btnSubmitText}"),
                        onPressed: () async {
                  ...
                  Expanded(
                      child: ElevatedButton.icon(
                        ...
                        label: pmBtnLabel("Detail List Ban"),
                        onPressed: () async {
                          ...
                          builder: (BuildContext context) => new AlertDialog(
                            ...
                            actions: <Widget>[
                              new ElevatedButton.icon(
                                ...
                                label: pmBtnLabel("Searh Partname"),
                                onPressed: () async {
                                  Navigator.of(context, rootNavigator: true)
                                      .pop();
                                  txtPartName.text = "";
                                  await _openItemSearchDialog(
                                      search: txtPartName.text);
                                },
                                style: ElevatedButton.styleFrom(
                                    elevation: 0.0,
                                    backgroundColor: primaryOrange,
                                            ...
                                            children: [
                                              Expanded(
                                                  child: ElevatedButton.icon(
                                                    ...
                                                    label: pmBtnLabel("Approve"), //Approve Opname
                                                    onPressed: () async {
                                                      //selEstimasi = "1";
                                                      //id_header=88;
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          dataListItemSearch = (jsonDecode(response.body) as List)
              .map((dynamic e) => e as Map<String, dynamic>)
              .toList();
          // var jsonData = {"id_detail": "18154", "item_id": "00-66-06-01", "quantity": 1, "merk": "HINO", "type": "GENUINE", "accessories": "STD", "part_name": "ALTERNATOR ASSY", "genuine_no": "27060-E0270", "item_size": "12 VOLT", "id_header": "16308"};
          // var jsonData1 = {"id_detail": "18155", "item_id": "00-66-06-01", "quantity": 1, "merk": "HINO", "type": "GENUINE", "accessories": "STD", "part_name": "ALTERNATOR ASSY", "genuine_no": "27060-E0270", "item_size": "12 VOLT", "id_header": "16308"};
          // var jsonData2 = {"id_detail": "18156", "item_id": "00-66-06-01", "quantity": 1, "merk": "HINO", "type": "GENUINE", "accessories": "STD", "part_name": "ALTERNATOR ASSY", "genuine_no": "27060-E0270", "item_size": "12 VOLT", "id_header": "16308"};
          // var jsonData3 = {"id_detail": "18157", "item_id": "00-66-06-01", "quantity": 1, "merk": "HINO", "type": "GENUINE", "accessories": "STD", "part_name": "ALTERNATOR ASSY", "genuine_no": "27060-E0270", "item_size": "12 VOLT", "id_header": "16308"};
          // var jsonData4 = {"id_detail": "18158", "item_id": "00-66-06-01", "quantity": 1, "merk": "HINO", "type": "GENUINE", "accessories": "STD", "part_name": "ALTERNATOR ASSY", "genuine_no": "27060-E0270", "item_size": "12 VOLT", "id_header": "16308"};
          // var jsonData5 = {"id_detail": "18159", "item_id": "00-66-06-01", "quantity": 1, "merk": "HINO", "type": "GENUINE", "accessories": "STD", "part_name": "ALTERNATOR ASSY", "genuine_no": "27060-E0270", "item_size": "12 VOLT", "id_header": "16308"};
          // var jsonData6 = {"id_detail": "181589", "item_id": "00-66-06-01", "quantity": 1, "merk": "HINO", "type": "GENUINE", "accessories": "STD", "part_name": "ALTERNATOR ASSY", "genuine_no": "27060-E0270", "item_size": "12 VOLT", "id_header": "16308"};
          // var jsonData7 = {"id_detail": "181587", "item_id": "00-66-06-01", "quantity": 1, "merk": "HINO", "type": "GENUINE", "accessories": "STD", "part_name": "ALTERNATOR ASSY", "genuine_no": "27060-E0270", "item_size": "12 VOLT", "id_header": "16308"};
          // var jsonData8 = {"id_detail": "181857", "item_id": "00-66-06-01", "quantity": 1, "merk": "HINO", "type": "GENUINE", "accessories": "STD", "part_name": "ALTERNATOR ASSY", "genuine_no": "27060-E0270", "item_size": "12 VOLT", "id_header": "16308"};

          // dataListItemSearch.add(jsonData);
          // dataListItemSearch.add(jsonData1);
          // dataListItemSearch.add(jsonData2);
          // dataListItemSearch.add(jsonData3);
          // dataListItemSearch.add(jsonData4);
          // dataListItemSearch.add(jsonData5);
          // dataListItemSearch.add(jsonData6);
          // dataListItemSearch.add(jsonData7);
          // dataListItemSearch.add(jsonData8);
        });
      } else {
            ...
            Container(
                child: ListView.builder(
                    ...
                    padding: const EdgeInsets.only(
                        top: 5.0, left: 5.0, right: 5.0, bottom: 300),
          ...
          child: Row(children: <Widget>[
            Expanded(
              ...
              onPressed: () async {
                ...
                await _showOpnameDetailDialog(context);
              },
              style: ElevatedButton.styleFrom(
                  elevation: 0.0,
                  backgroundColor: primaryOrange,
