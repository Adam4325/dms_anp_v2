...
class ViewListWoMCN extends StatefulWidget {
  @override
  _ViewListWoMCNState createState() => _ViewListWoMCNState();
}

class _ViewListWoMCNState extends State<ViewListWoMCN> {
  ...
  Future<String> stopWO(String wodwonumber, String loginname, String userid,
    ...
    try {
      ...
      setState(() {
        ...
        status_code = responseData["status_code"]?.toString() ?? "";
        message = responseData["message"]?.toString() ?? "";
        if (int.tryParse(status_code) == 200) {
  ...
  Future<bool> startWO(String wodwonumber, String loginname, String userid,
      String mechanicid) async {
    var isSaved = false;
    if (pr?.isShowing() == false) {
      await pr?.show();
    ...
    try {
      var urlData =
          "${GlobalData.baseUrl}api/maintenance/wo_mcn_start_stop.jsp?method=wo-mcn&act=start-wo-detail" +
              "&wodwonumber=" +
              wodwonumber +
              "&loginname=" +
              loginname +
  ...
  Future<String> getJSONData() async {
    ...
    setState(() {
      url =
          "${GlobalData.baseUrl}api/maintenance/list_wo_start2.jsp?method=list-wo&is_barcode=0&locid=" +
              locid +
              "&mechanicid=" +
              mechanicid;
    });
              ...
              actions: <Widget>[
                ...
                ElevatedButton.icon(
                  ...
                  onPressed: () async {
                    ...
                    String _wodwonbr = value['wodwonbr']?.toString() ?? "";
                    ...
                    if (_wodwonbr.isEmpty) {
                      ...
                      if (saved == true) {
                        Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(
                                builder: (context) => ViewListWoDetailMCN()));
                      }
                    }
                  },
                  style: ElevatedButton.styleFrom(
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
            ...
            centerTitle: true,
            title: Text('Mechanic List Work Orders',
                style: TextStyle(
                    color: Colors.white,
  ...
  Future scanQRCodeWO(dynamic value) async {
    ...
    return;
    // Original: String cameraScanResult = await scanner.scan();
    /*
    setState(() {
      scanResult = cameraScanResult ?? "";
      print("scanResult : $scanResult");
      if (scanResult.isNotEmpty) {
        var wonumber = scanResult;
        global_wo_number = wonumber;
        if (global_wo_number.isNotEmpty) {
          ...
          children: <Widget>[
            ...
            Container(
              padding: EdgeInsets.only(
                  left: 5.0, right: 15.0, top: 12.0, bottom: 10),
              child: Row(
