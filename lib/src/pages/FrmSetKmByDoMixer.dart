...
class FrmSetKmByDoMixer extends StatefulWidget {
  final String? vehilce;
  final String? vhckm;
  final String? bujnbr;
  final String? do_number;
  final String? driver_id;
  const FrmSetKmByDoMixer(
      {Key? key,
      this.vehilce,
      this.vhckm,
      this.bujnbr,
      ...
      : super(key: key);
  @override
  _FrmSetKmByDoMixerState createState() => _FrmSetKmByDoMixerState();
}

final globalScaffoldKey = GlobalKey<ScaffoldState>();

class _FrmSetKmByDoMixerState extends State<FrmSetKmByDoMixer> {
  ...
  String _extractOdometerKm(String fullText) {
    ...
    final parts = digitsOnly
        .split(RegExp(r'\s+'))
        .where((s) => s.length >= 4 && s.length <= 8)
        .toList();
    if (parts.isEmpty) {
  ...
  Future<File?> _pickedImageToTempFile(XFile pickedFile) async {
    try {
      ...
      final path =
          '${dir.path}/odometer_${DateTime.now().millisecondsSinceEpoch}.jpg';
  ...
  Future<File> _preprocessImage(File file) async {
    try {
      ...
      if (bytes.length > 5 * 1024 * 1024) {
        debugPrint(
            'Gambar terlalu besar (${bytes.length} bytes), skip preprocessing');
        return file;
      }
      ...
      if (image.width > 2000) {
        ...
        processedImage = img.copyResize(image,
            width: 2000, height: (image.height * ratio).toInt());
      }
      ...
      final dir = await getTemporaryDirectory();
      final outFile = File(
          '${dir.path}/odometer_ocr_${DateTime.now().millisecondsSinceEpoch}.jpg');
    ...
    try {
      ...
      if (imageFile == null || !imageFile.existsSync()) {
        ...
        alert(globalScaffoldKey.currentContext!, 0,
            'Gagal menyimpan foto. Coba lagi.', 'error');
        ...
        final ocrText = await _recognizeText(imageFile).timeout(
          ...
          onTimeout: () {
            throw TimeoutException(
                'Proses OCR terlalu lama. Coba foto ulang dengan pencahayaan lebih baik.');
          },
        );
      ...
      if (ocrError != null && mounted) {
        alert(globalScaffoldKey.currentContext!, 0,
            'Gagal baca KM: $ocrError. Silakan input manual.', 'error');
      }

      if (mounted) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted)
            _showKmConfirmDialog(ocrKm.isNotEmpty ? ocrKm : txtKM.text);
        });
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<int> getCountHaveDo() async {
    ...
    vhcid = widget.vehilce ?? "";
    if (vhcid.isNotEmpty) {
      locid = prefs.getString("locid")!;
      String bujnumber = widget.bujnbr!;
      var urlData =
          "${GlobalData.baseUrl}api/do_mixer/last_antrian.jsp?method=get-total-do&vhcid=${vhcid}&locid=${locid}&bujnumber=${bujnumber}";
  ...
  void updateKM(String vhckm, int isBack) async {
    ...
    vhcid = widget.vehilce!; // prefs.getString("vhcidfromdo") ?? "";
    if (vhcid.isNotEmpty) {
      drvid = widget.driver_id!; // prefs.getString("drvid")!;
      ...
      setState(() {
        ...
        if (response.statusCode != 200) {
          alert(globalScaffoldKey.currentContext!, 0, 'Gagal update ${message}',
              'error');
          return;
        }
        if (status_code != null && status_code == "200") {
          ...
          if (prefs.getString("login_type") == "MIXER") {
            prefs.setString("km_mixer", widget.vhckm!);
            prefs.setString("vhcid_mixer", widget.vehilce!);
            prefs.setString("bujnumber_mixer", widget.bujnbr!);
            prefs.setString("do_numbe_mixer", widget.do_number!);
            prefs.setString("driver_id_mixer", widget.driver_id!);
          }
  ...
  void getVehiceldriverBuj() async {
    ...
    var driver_id = widget.driver_id; // prefs.getString("drvid");
    var bujnumber = widget.bujnbr; // prefs.getString("bujnumber");
    print(bujnumber);
  ...
  void updateKMStandby(
    ...
    var _vhcid = vhcid == null && vhcid == '' ? widget.vehilce : vhcid;
    //vhcid = 'B 9189 KYW';
  ...
  Future<String?> GetListVehicleDoDiTerima() async {
    String km = "0";
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String drvid = widget.driver_id!; // prefs.getString('drvid')!;
    try {
      ...
      var urlData =
          "${GlobalData.baseUrl}api/do_mixer/do_diterima_mixer.jsp?method=list-set_vehicle_diterima_apps&drvid=${drvid}";
      var encoded = Uri.encodeFull(urlData);
      print(urlData);
      Uri myUri = Uri.parse(encoded);
      var response =
      ...
      if (response.statusCode == 200) {
        ...
        if (status_code == 200) {
          var vhcid = widget.vehilce!;
          var bujnumber = widget.bujnbr!;
          // setState(() {
  ...
  Future<void> _initParamsFromWidget() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (widget.vehilce != null && widget.vehilce!.isNotEmpty) {
      txtNopol.text = widget.vehilce!;
      vhcid = widget.vehilce!;
      prefs.setString("vhcidfromdo", widget.vehilce!);
    }
    if (widget.vhckm != null && widget.vhckm!.isNotEmpty) {
      txtKMOld.text = widget.vhckm!;
      vhckm = widget.vhckm!;
    }
    if (widget.bujnbr != null && widget.bujnbr!.isNotEmpty) {
      prefs.setString("bujnumber", widget.bujnbr!);
    }
    if (widget.driver_id != null && widget.driver_id!.isNotEmpty) {
      prefs.setString("drvid", widget.driver_id!);
    }
  ...
  Widget _getVehicleListOLD(BuildContext context) {
    print("dataListVehicle");
    print(dataListVehicle);
                                ...
                                child: SizedBox(
                                  ...
                                  child: ElevatedButton.icon(
                                    ...
                                    onPressed: () async {
                                      ...
                                      String km_awal =
                                          txtKMOld.value.text.toString();
                                      String km_new =
                                          txtKM.value.text.toString();
                                      ...
                                      if (vhcidNew == null || vhcidNew == "") {
                                        alert(
                                            ...
                                            "warning");
                                      } else if (km_awal == null ||
                                          km_awal == "") {
                                        alert(context, 2,
                                            "KM tidak boleh kosong", "warning");
                                      } else if (km_new == null ||
                                          km_new == "") {
                                        alert(context, 2,
                                            "KM tidak boleh kosong", "warning");
                                        ...
                                        alert(
                                            context,
                                            2,
                                            "KM Awal tidak boleh kosong",
                                            "warning");
                                      } else if (int.parse(km_new) <= 0) {
                                        alert(
                                            context,
                                            2,
                                            "KM Akhir tidak boleh kosong",
                                            "warning");
                                        ...
                                        alert(
                                            context,
                                            2,
                                            "KM Akhir harus > dari KM Awal",
                                            "warning");
                                      } else if (bujnumber == null ||
                                          bujnumber == '') {
                                        alert(
                                            context,
                                            2,
                                            "Buj Number tidak boleh kosong",
                                            "warning");
                                      } else {
                                        SharedPreferences prefs =
                                        ...
                                        if (prefs.getString("vhcidfromdo") ==
                                                null ||
                                  ...
                                  child: ElevatedButton.icon(
                                    ...
                                    onPressed: () async {
                                      ...
                                      if (totalHaveDo > 0) {
                                        ...
                                        String vhcidNew = widget.vehilce!;
                                        String km_awal = widget.vhckm!;
                                        String km_new =
                                            txtKM.value.text.toString();
