...
class _ViewDashboardState extends State<ViewDashboard>
  ...
  // âœ… ADDED: Variables untuk menu management
  ...
  // âœ… ADDED: Orange Soft Color Scheme
  ...
  bool _shouldSplitMenus() =>
      (username == "ADMIN" || getAkses("OP")) &&
      _anpServiceList.length > MAX_MAIN_MENU;

  void _applySavedMenuOrder() {
    final saved = sharedPreferences?.getStringList(_menuOrderPrefKey());
    ...
    final known = <AnpService>[];
    final unknown = <AnpService>[];
    for (final item in _anpServiceList) {
      final id = item.idKey;
  ...
  Future<void> _saveMenuOrder(List<AnpService> ordered) async {
    ...
    final ids = ordered
        .where((e) => e.idKey != null && e.idKey != 999)
        .map((e) => e.idKey.toString())
        .toList();
    await prefs.setStringList(_menuOrderPrefKey(), ids);
  ...
  Future<void> _resetMenuOrder() async {
    final prefs = sharedPreferences ?? await SharedPreferences.getInstance();
    await prefs.remove(_menuOrderPrefKey());
    if (!mounted) {
      return;
    }
    setState(() {
  ...
  void _showMenuCustomizer() {
    if (_anpServiceList.length < 2) {
      return;
    }

    final editable = List<AnpService>.from(_anpServiceList);
    final splitMenus = _shouldSplitMenus();
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Text(
                              splitMenus
                                  ? 'Drag untuk ubah urutan. $MAX_MAIN_MENU menu pertama tampil di Menu Utama.'
                                  : 'Drag untuk ubah urutan menu.',
                              style: TextStyle(
                    ...
                    Expanded(
                      child: ReorderableListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final service = editable[index];
                          final isMainSlot =
                              splitMenus && index < MAX_MAIN_MENU;
                          return Card(
                            key: ValueKey('menu_${service.idKey}'),
                            ...
                            child: ListTile(
                              leading: Container(
                                ...
                                decoration: BoxDecoration(
                                  color: (service.color ?? Colors.grey)
                                      .withOpacity(0.1),
                            ...
                            onPressed: () async {
                              ...
                              if (mounted) {
                                ScaffoldMessenger.of(this.context).showSnackBar(
                                  SnackBar(
                                    content: Text('Urutan menu disimpan'),
                                  ),
                                );
                              }
                            },
  ...
  void _setupNotifications() {
    print('ðŸ” DEBUG: Setting up notifications in ViewDashboard...');
    ...
    _notificationSubscription = _notificationService.notificationStream.listen(
      (notifications) {
        print(
            'ðŸ” DEBUG: Stream received ${notifications.length} notifications');
        setState(() {
      ...
      onError: (error) {
        print('âŒ ERROR: Stream error: $error');
      },
      onDone: () {
        print('ðŸ” DEBUG: Stream done');
      },
    );

    _notificationService.startNotificationTimer(intervalSeconds: 30);
    print('ðŸ” DEBUG: Notification setup completed');
  }
  ...
  void _showLatestNotification(List<NotificationData> notifications) {
    ...
    if (newNotification != null) {
      print('ðŸ” DEBUG: Showing new notification: ${newNotification.id}');
  ...
  void _showNotificationDialog(NotificationData notification) {
    print(
        'ðŸ” DEBUG: Attempting to show notification dialog for ID: ${notification.id}');

    if (_isDialogShowing) {
      print('âš ï¸ WARNING: Dialog already showing, skipping...');
      return;
    }

    if (!mounted || context == null) {
      print('âŒ ERROR: Context not available for dialog');
      return;
    }
    ...
    try {
      ...
      print('âœ… SUCCESS: Dialog shown successfully');
    } catch (e) {
      print('âŒ ERROR: Failed to show dialog: $e');
      _isDialogShowing = false;
    }
  }

  void _handleNotificationTap(NotificationData notification) async {
    print('ðŸ” DEBUG: Handling notification tap for: ${notification.id}');
    try {
      await _notificationService.sendNotificationResponse(notification);
      _notificationService.removeNotification(notification.id);
      print('âœ… SUCCESS: Notification handled successfully');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
      ...
      print('âŒ ERROR: Exception in _handleNotificationTap: $e');
    }
  }
  ...
  @override
  void dispose() {
    print('ðŸ” DEBUG: ViewDashboard dispose called');
    _notificationSubscription?.cancel();
    timer?.cancel();
  ...
  String _buildRunningInfoText() {
    ...
    final baseText = _runningInfoItems.join('     â€¢     ');
        ...
        children: [
          Row(
            children: [
              ...
              Spacer(),
              if (_anpServiceList.length > 1)
                IconButton(
                  tooltip: 'Atur Menu',
                  padding: EdgeInsets.zero,
                  constraints: BoxConstraints(),
                  icon: Icon(Icons.edit, color: primaryOrange, size: 20),
                  onPressed: _showMenuCustomizer,
                ),
              if (_anpServiceList.length > 1 && _additionalMenuList.isNotEmpty)
                SizedBox(width: 4),
  ...
  Widget _buildMenuCard(AnpService service) {
    // âœ… UPDATED: Special styling untuk menu "More"
            ...
            Container(
              ...
              child: Stack(
                ...
                children: [
                  ...
                  // âœ… ADDED: Badge untuk menu "More"
  ...
  // âœ… ADDED: Function untuk menampilkan menu tambahan
    ...
    return Column(
      children: [
        _buildInfoItem(Icons.directions_car, 'Status Unit/QR Pertamina',
            status_unit, primaryOrange, // âœ… UPDATED: Orange theme
        ...
        _buildInfoItem(Icons.assignment, 'STNK', '${fromSTNK}-${toSTNK}',
            accentOrange, // âœ… UPDATED: Orange theme
      ...
      children: [
        if (loginname == "MECHANIC")
          _buildInfoItem(Icons.handyman, 'Outstanding Jobs', out_standing_jobs,
              primaryOrange, // âœ… UPDATED: Orange theme
                ...
                children: [
                  Container(
                    ...
                    child: Icon(
                      ...
                      color: isActive
                          ? primaryOrange
                          : Colors.grey.shade600, // âœ… UPDATED: Orange theme
                      size: 20,
                    ),
                  ),
                  if (badge > 0)
        ...
        return AlertDialog(
                            ...
                            Row(
                              children: [
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: primaryOrange.withOpacity(
                                        0.2), // âœ… UPDATED: Orange theme
                                    borderRadius: BorderRadius.circular(4),
                                  ),
                                  child: Icon(Icons.star,
                                      color: primaryOrange,
                                      size: 16), // âœ… UPDATED: Orange theme
                                ),
                                SizedBox(width: 8),
                                Text(
                                  "${item['points']} Points",
                                  style: TextStyle(
                                    fontWeight: FontWeight.bold,
                                    color:
                                        darkOrange, // âœ… UPDATED: Orange theme
                                  ),
                                ),
                              ],
                            ),
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: primaryOrange, // âœ… UPDATED: Orange theme
                shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(8)),
              ),
              onPressed: () async {
  ...
  void _handleMenuTap(AnpService anpService) async {
    // âœ… ADDED: Handle menu "More"
    if (anpService.idKey == 999) {
      _showMoreMenuModal();
      return;
    }

    // âœ… Original menu handling logic remains the same
    ...
    if (anpService.idKey == 1) {
      ...
      if (loginname == "DRIVER") {
        ...
        // print("ðŸ” Token result driver: ${_tokens}");
        // print("ðŸ” Token length: ${_tokens?.length}");
        // print("ðŸ” Token type: ${_tokens.runtimeType}");
      ...
      if (ctx != null) {
        await showDialog(
          ...
          builder: (context) => new AlertDialog(
            ...
            actions: <Widget>[
              new TextButton(
                // âœ… UPDATED: Use TextButton instead of FlatButton
                onPressed: () async {
                  Navigator.of(context).pop(true);
                },
                child: new Text('Ok'),
