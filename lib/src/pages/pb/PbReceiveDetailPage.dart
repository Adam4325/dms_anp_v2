...
/// Detail baris PB Receive (V_PBRECEIVE_DETAILDMS).
class PbReceiveDetailPage extends StatefulWidget {
  final String itxinvtrannbr;

  const PbReceiveDetailPage({Key? key, required this.itxinvtrannbr})
      : super(key: key);

  @override
  State<PbReceiveDetailPage> createState() => _PbReceiveDetailPageState();
}

class _PbReceiveDetailPageState extends State<PbReceiveDetailPage> {
  List<dynamic> _lines = [];
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final url = Uri.parse('${GlobalData.baseUrl}api/pb/pb_receive_detail.jsp')
                          ...
                          return Container(
                            ...
                            child: ExpansionTile(
                              ...
                              children: [
                                ...
                                _kv('typepb', _cell(row, ['typepb', 'typepb'])),
                                _kv('PBNBR', _cell(row, ['PBNBR', 'pbnbr'])),
                                _kv('ITDLINENBR',
